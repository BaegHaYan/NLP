...
class Persona_classification(torch.nn.Module):
    def __init__(self, hparams: argparse.Namespace):
        ...
        self.embed_layer = torch.nn.Embedding(self.input_dim, self.embedding_dim, padding_idx=self.pad_token_id)
        ...
        self.transformer_encoder_layer = torch.nn.TransformerEncoder(transformer_encoder, num_layers=self.num_layers, norm=torch.nn.LayerNorm(self.hidden_dim))
        self.output_layer = torch.nn.Sequential(
            torch.nn.Linear(self.hidden_dim, int(self.hidden_dim/2)),
            torch.nn.Tanh(),
            torch.nn.LayerNorm(int(self.hidden_dim/2)),
            torch.nn.Linear(int(self.hidden_dim/2), self.hidden_dim),
            torch.nn.Tanh(),
    ...
    def add_model_argments(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument("-e", type=int, default=1024, dest="embedding_dim", help="embedding size")
        parser.add_argument("-h", type=int, default=786, dest="hidden_dim", help="hidden size")
...
if __name__ == "__main__":
    ...
    lr_scheduler = SequentialLR([constant_scheduler, exponential_scheduler], milestones=[int(hparams.epochs * hparams.warmup_rate)])
    ...
    for i in range(hparams.epochs):
        for j, batch in enumerate(train_loader):
            ...
            logger.info(f"Epochs {i+1} : loss - {loss.item()}, acc - {acc.item()} | {j}/{len(train_loader)}({j/len(train_loader) * 100}%)\r")
        ...
        with torch.no_grad():
            ...
            for batch in val_loader:
                ...
                val_acc.append(accuracy(pred, Y).item())
            val_loss = torch.mean(torch.tensor(val_loss))
            val_acc = torch.mean(torch.tensor(val_acc))
            logger.info(f"Epochs {i+1} : val_loss - {val_loss.item()}, val_acc - {val_acc.item()})")
